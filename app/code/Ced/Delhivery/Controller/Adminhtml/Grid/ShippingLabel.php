...
class ShippingLabel extends \Magento\Backend\App\Action
{
    ...
    {   
		...
		if (!empty($wayBills))
		{
			...
			{	
				$orderid ='';
				$awb = $this->_objectManager->create('Ced\Delhivery\Model\Awb')->load(trim($_wayBills),'awb')->getData();
				if(empty($awb))
				{
					continue;				
				}
					...
					foreach ($shipments as $shipment)
		  			{
		  				if(($shipment->getOrder()!=NULL)&&($shipment->getIncrementId()==$shipment_id)){
		  					...
		  					$this->_objectManager->get('\Ced\Delhivery\Model\ShippingLabel')->getContent($pdf->pages[$pagecounter], $shipment->getEntityId(), $_wayBills, $shipment->getOrder(),$y);
		  				}
		  			}
				}
				$y = $y-190;				
