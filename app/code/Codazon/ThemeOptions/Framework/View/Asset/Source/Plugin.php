...
class Plugin
{
    ...
    public function aroundGetContent($subject, $procede, $asset)
    {
        ...
        if (!$this->state->isAreaCodeEmulated()) { // generate static by browser
            if (strpos((string)$path, 'frontend/Codazon') !== false && strpos((string)$path, '.less') !== false) {
                if (strpos((string)$path, 'source/_variables.less') !== false) {
                    $this->loadData($asset);
                    $data = $this->config;
                    ...
                    foreach ($this->data as $key => $value) {
                        ...
                        if (strpos((string)$value, '#') !== false) {
                            $result .= '@' . $key . ':' . $value . '; ';
                        } elseif (strpos((string)$value, 'rgba(') !== false) {
                            $result .= '@' . $key . ':' . $value . '; ';
                        } else {
